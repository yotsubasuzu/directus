...
import { useApi, useLayout, useStores } from '@directus/extensions-sdk';
...
import { Folder, useFolders } from '../core-clones/composables/use-folders';
import { useCollectionPermissions } from '../core-clones/composables/use-permissions';
import { usePreset } from '../core-clones/composables/use-preset';
import emitter, { Events } from '../core-clones/events';
import { getFolderFilter } from '../core-clones/utils/get-folder-filter';
import { unexpectedError } from '../core-clones/utils/unexpected-error';
import { uploadFiles } from '../core-clones/utils/upload-files';
	...
	async function moveToFolder() {
		...
		try {
			...
			if (selectedFolder.value) {
				router.push(`/module-files/folders/${selectedFolder.value}`);
			}
		...
		>
			...
			<template #actions>
				...
				<v-button
					...
					rounded
					icon
					:to="folder ? { path: `/module-files/folders/${folder}/+` } : { path: '/module-files/+' }"
			...
			<component :is="`layout-${layout}`" v-bind="layoutState">
				<template #no-results>
					<v-info v-if="!filter && !search" :title="t('file_count', 0)" icon="folder" center>
						...
						<template #append>
							<v-button :to="folder ? { path: `/module-files/folders/${folder}/+` } : { path: '/module-files/+' }">
								{{ t('add_file') }}
							</v-button>
						</template>
					</v-info>
